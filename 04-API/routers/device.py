...
# 5. SSE currently device status stream
...
async def stream_currently_device(
    ...
    """
    SSE stream yielding currently device status ratio segments for a specific device.
    Fires every 30 seconds.
    """
    async def event_generator():
        logger.info(f"SSE currently device status stream started for device '{process}/{device}'")
        try:
            while True:
                now = get_now_bangkok()
                start_time, end_time = get_production_day_range(now)
            ...
            logger.info(f"SSE currently device status stream cancelled for device '{process}/{device}'")
            raise
        except Exception as e:
            logger.error(f"Error in SSE currently device status stream: {e}")
            yield f"event: error\ndata: {str(e)}\n\n"

    return StreamingResponse(event_generator(), media_type="text/event-stream")


# 6. SSE daily device status stream
...
async def stream_daily_device(
    ...
    """
    SSE stream yielding daily device status ratio segments for a specific device.
    Fires every 30 seconds.
    """
    async def event_generator():
        logger.info(f"SSE daily device status stream started for device '{process}/{device}'")
        try:
            while True:
                now = get_now_bangkok()
                if now.hour < 7:
                ...
                daily_devices = group_device_by_prod_date(records, [device], start_date, end_date, initial_statuses)
                
                response_list = []
                for d in daily_devices:
            ...
            logger.info(f"SSE daily device status stream cancelled for device '{process}/{device}'")
            raise
        except Exception as e:
            logger.error(f"Error in SSE daily device status stream: {e}")
            yield f"event: error\ndata: {str(e)}\n\n"
