<script lang="ts">
	...
	$effect(() => {
		...
		// Only reset if process actually changed
		selectedDevices = [];
		timelines = {};
	});

	let timelines = $state<Record<string, any[]>>({});
	let loading = $state(false);
	let error = $state<string | null>(null);
	let pollTimer: ReturnType<typeof setInterval> | null = null;

	async function fetchAllTimelines(p: string, devices: string[], maxRetries = 3, baseDelayMs = 2000) {
		...
		for (let attempt = 1; attempt <= maxRetries; attempt++) {
			try {
				...
				// Batch response: { "no_1": [...segments], "no_2": [...segments], ... }
				const json = await res.json();

				// Merge into existing timelines (keep stale data for devices not in response)
				timelines = { ...untrack(() => timelines), ...json };
				error = null;
				loading = false;
				return;
			} catch (err: any) {
				...
				if (attempt < maxRetries) {
					...
					console.warn(`[Timeline] Batch fetch failed (attempt ${attempt}/${maxRetries}), retrying in ${wait}ms…`);
		...
		console.error('[Timeline] Batch fetch failed after retries:', lastError?.message);
		error = lastError?.message ?? 'Failed to fetch timeline data';
		loading = false;
	}
	...
	$effect(() => {
		...
		if (selectedProcess && selectedDevices.length > 0) {
			// Snapshot to avoid tracking inside async callback
			const p = selectedProcess;
			const devs = [...selectedDevices];

			fetchAllTimelines(p, devs);

			// Refresh every 30 seconds
			pollTimer = setInterval(() => fetchAllTimelines(p, devs), 30_000);
		} else {
			timelines = {};
		}
...
<div class="space-y-6">
	...
	<div class="rounded-lg border border-border bg-card overflow-hidden">
		...
		{#if selectExpanded}
			<div class="p-4 border-t border-border bg-card/50">
				<div
					class="flex flex-wrap gap-2 max-h-40 overflow-y-auto p-1.5 border border-border/50 rounded bg-background/50"
				>
	...
	<!-- Status Color Legend -->
	<div
		class="flex flex-wrap items-center gap-4 px-4 py-2 border border-border rounded-lg bg-card text-xs"
	>
		...
		{#if selectedDevices.length === 0}
			<div class="rounded-lg border border-dashed p-12 text-center">
				<p class="text-sm text-muted-foreground">
					No machines selected. Please select up to 5 machines to display timelines.
				</p>
			</div>
		{:else}
			{#each selectedDevices as dev}
				<div
					class="rounded-lg border border-border bg-card p-2 px-4 hover:shadow-sm transition-all duration-200 flex items-center gap-6"
				>
					...
					<div class="w-32 shrink-0 flex flex-col gap-1">
						...
						<span
							class="w-fit rounded border border-border px-1.5 py-0.5 text-[9px] font-medium uppercase text-muted-foreground"
						>
							{selectedProcess}
						</span>
					</div>
					...
					<div class="flex-1 min-w-0">
						<TimelineChart segments={timelines[dev] || []} {colorMap} {loading} {error} />
					</div>
				</div>
			{/each}
		{/if}
